...
import torch
import util
from eval.evalRGGE import valid, test, average_precision, precision_at_k
...
def main(DG, DE, args):
    ...
    A = torch.sparse_coo_tensor(edge_index.t(), edge_weight.squeeze(),
                                size=(num_nodes, num_nodes)).to_dense()  # Corresponding G0
    ...
    B = torch.sparse_coo_tensor(edge_index.t(), edge_weight.squeeze(),
                                size=(num_nodes, num_nodes)).to_dense()  # Corresponding G
    ...
    loss_weight_optimizer = torch.optim.Adam([
    ...
    for epoch in epoch_iterator:
        ...
        if valid_aupr > best_valid_aupr:
            ...
            torch.save(model.state_dict(), args.result_fold + f'model_checkpoint_{args.fold}.pth')
    ...
    test(model, DG['test']['edge'], DG['test']['edge_neg'], DE['test']['edge'], DE['test']['edge_neg'], args, log_file)


if __name__ == "__main__":
    args = parse_args()
    split_DG, split_DE = util.split_dataset(args.k_fold, args.random_seed)
    args.result_fold = os.path.join(os.getcwd(), f"results/")
    ...
    for (pos_train0, neg_train0, pos_val0, neg_val0, pos_test0, neg_test0,
        ...
        log_file = args.result_fold + f'log{args.fold}.txt'
        print(log_file)
